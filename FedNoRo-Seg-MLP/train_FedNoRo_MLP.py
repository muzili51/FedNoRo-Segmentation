...
def calculate_confidence_mask(logits_a, logits_b):
    ...
    conf_a, _ = torch.max(prob_a, dim=1, keepdim=True)
    conf_b, _ = torch.max(prob_b, dim=1, keepdim=True)

    learn_from_b_mask = (conf_b > conf_a).float()
...
def calculate_reliability_weight(features, pseudo_labels, num_classes, epsilon=1e-8):
    ...
    for b in range(B):
        feat = features[b]
        label = pseudo_labels[b]
        prototypes = []
        valid_classes = []
        for cls in range(num_classes):
            mask = (label == cls)
            if mask.sum() > 0:
                cls_features = feat[:, mask]
                prototype = cls_features.mean(dim=1)
        ...
        sim_matrix = F.cosine_similarity(feat_flat.unsqueeze(1), prototypes.unsqueeze(0), dim=2) 
        sim_matrix = sim_matrix.view(H, W, num_classes).permute(2, 0, 1)
        pseudo_labels_flat = label.view(-1)
        ...
        reliability_maps.append(reliability_map.unsqueeze(0))

    reliability_weight = torch.stack(reliability_maps, dim=0)
...
def compute_hr_weight(features, pseudo_labels, num_classes, device, epsilon=1e-8):
    ...
    for b in range(B):
        ...
        proto_for_pixel = prototypes[label_flat]

        cos_sim = F.cosine_similarity(feat_flat, proto_for_pixel, dim=1) 
        hr_map = cos_sim.view(1, H, W)
        hr_maps.append(hr_map)

    return torch.stack(hr_maps, dim=0)
...
def advanced_federated_training_with_dual_networks(args, server, clients):
    ...
    for round_num in range(args.s1, args.rounds):
        ...
        for client in clients:
            ...
            if client.client_id in detected_noisy and hasattr(client, 'model1'):
                # 只加载到 model1 (UNet)，model2 (TransformerUNet) 保持独立
                client.model1.load_state_dict(global_model.state_dict())
                # model2 不加载全局权重，或使用兼容的权重
                avg_loss = dual_network_train_local(client, args.local_ep, args.base_lr)
                client_models.append(client.model1)  # 只聚合 model1
            else:
                client.model.load_state_dict(global_model.state_dict())
...
if __name__ == "__main__":
    ...
    image_folder = "D:\\FedNoRo-Seg\\Fundus-doFE\\Fundus\\Domain1\\train\\image"
    mask_folder = "D:\\FedNoRo-Seg\\Fundus-doFE\\Fundus\\Domain1\\train\\mask"
    ...
    client_loaders, test_loader = create_federated_dataloaders(
        ...
        image_size=(256, 256),
        noise_ratio=0.8,
        noise_client_ratio=args.noise_client_ratio
    )
    ...
    for i in range(num_clients):
        if client_loaders[i]['has_noise']:
            # 噪声客户端使用双网络
            model1 = get_model(args.model, args.pretrained)  # UNet
            model2 = get_model(args.model, args.pretrained)  # UNet (与 model1 结构一致)
            # 或者 model2 使用 TransformerUNet，但不参与聚合
            
            client = DualNetworkClient(
                client_id=i,
                model1=model1,
                model2=model2,
            ...
            # 干净客户端使用单网络
            client_model = get_model(args.model, args.pretrained)
            client = Client(
                client_id=i,
                model=client_model,
                train_loader=client_loaders[i]['train'],

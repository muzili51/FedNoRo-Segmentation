...
# --- 修改 Client 类 ---
class Client:
    def __init__(self, client_id, model, train_loader, val_loader, device, class_freqs, tau=1.0):
    ...
    def train_local(self, epochs=5, learning_rate=0.001):
        ...
        self.model.train()
        # --- 修改：使用新的损失函数 ---
        criterion = LogitAdjustedCrossEntropyLoss(self.class_freqs, self.tau)
...
def calculate_global_class_frequencies(clients, num_classes, device):
    """
    计算所有客户端数据的全局类别频率 (π_y)。
    ...
    Returns:
        global_freqs: 形状为 [num_classes] 的张量，包含每个类别的全局频率。
    """
    global_pixel_counts = torch.zeros(num_classes, dtype=torch.long, device=device)
    
    # 遍历每个客户端，累加其本地的像素计数
    for client in clients:
        local_pixel_counts = torch.zeros(num_classes, dtype=torch.long, device=device)
        for _, masks in client.train_loader:
            masks = masks.to(device)
            # 使用 bincount 计算每个类别的像素数量
            counts = torch.bincount(masks.flatten(), minlength=num_classes)
            local_pixel_counts += counts
        
        global_pixel_counts += local_pixel_counts
    
    # 计算频率 π_y = N_y / N_total
    total_pixels = global_pixel_counts.sum()
    global_freqs = global_pixel_counts.float() / total_pixels
    
    # 防止 log(0)，用一个极小值代替0频率（理论上分割任务中所有类别都应出现）
    global_freqs = torch.clamp(global_freqs, min=1e-8)
    
    print(f"计算得到的全局类别频率: {global_freqs.cpu().numpy()}")
    return global_freqs


# --- 新增类：Logit-Adjusted Cross-Entropy Loss ---
class LogitAdjustedCrossEntropyLoss(nn.Module):
    """
    基于论文 'Long-Tail Learning via Logit Adjustment' 实现的损失函数。
    """
    def __init__(self, class_freqs, tau=1.0):
        """
        Args:
            class_freqs: 形状为 [num_classes] 的张量，包含每个类别的频率 π_y。
            tau: 调整参数，默认为1.0。
        """
        super().__init__()
        self.class_freqs = class_freqs
        self.tau = tau
        # 预计算 log(π_y^τ) 作为偏置项，并将其移动到与 class_freqs 相同的设备
        self.logit_bias = (self.tau * torch.log(self.class_freqs)).to(self.class_freqs.device)

    def forward(self, logits, targets):
        """
        Args:
            logits: 模型原始输出，形状 [N, C, H, W]。
            targets: 真实标签，形状 [N, H, W]。
        """
        # 将logits和targets展平以适应标准交叉熵
        # logits: [N*H*W, C]
        # targets: [N*H*W]
        ...
        # 动态地将 logit_bias 移动到与 logits_flat 相同的设备
        logit_bias = self.logit_bias.to(logits_flat.device)
        
        # 应用logit调整: logits_adj = logits - τ * log(π_y)
        adjusted_logits = logits_flat + (-logit_bias.unsqueeze(0))  # [N*H*W, C]

        # 计算标准交叉熵
        loss = F.cross_entropy(adjusted_logits, targets_flat, ignore_index=-1)
        return loss
...
if __name__ == "__main__":
    ...
    # 创建全局模型架构（权重稍后会被重置）
    global_model_template = UNet(n_channels=3, n_classes=num_classes)
    ...
    model_save_dir = "saved_models_for_fedLA"
    ...
    for noise_rate in noise_rates:
        ...
        # 2. 创建临时客户端以计算全局频率
        # 注意：这里使用模板模型，因为我们只关心数据加载器
        temp_clients = []
        for i in range(num_clients):
            temp_client = Client(
                client_id=i,
                model=global_model_template, # 临时模型
                train_loader=client_loaders[i]['train'],
                val_loader=client_loaders[i]['val'],
                device=device,
                class_freqs=torch.ones(num_classes), # 临时占位符
                tau=1.0
            )
            temp_clients.append(temp_client)

        # 3. 计算全局类别频率 π_y
        global_class_freqs = calculate_global_class_frequencies(temp_clients, num_classes, device)
        # 将频率移到CPU，以便后续在创建新Client时可以安全传递（避免GPU内存问题）
        global_class_freqs = global_class_freqs.cpu()

        # 4. 创建正式的客户端实例
        clients = []
        for i in range(num_clients):
            client_model = UNet(n_channels=3, n_classes=num_classes)
            ...
            client = Client(
                ...
                val_loader=client_loaders[i]['val'],
                device=device,
                class_freqs=global_class_freqs, # 使用全局频率
                tau=1.0 # 可以根据需要调整tau
            )
            clients.append(client)

        # 5. 创建联邦学习服务器
        server = FedAvgServer(global_model_template, clients, device)
        ...
        model_save_path = os.path.join(model_save_dir, f"fedLA_model_noise_{noise_rate}.pth")
        torch.save(trained_global_model.state_dict(), model_save_path)
        print(f"模型已保存至: {model_save_path}")

        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        reset_model_parameters(global_model_template)